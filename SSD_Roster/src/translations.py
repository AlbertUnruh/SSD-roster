...
class TranslationNamespace:
    ...
    def tn_get_language(self, lan: str) -> dict[str, Any]:
        if lan not in Settings.LANGUAGE_AVAILABLE:
    ...
    def tn_get_translation(self, key: str, lan: str = None) -> dict | str:
        if lan is None:
            lan = language.get(Settings.LANGUAGE_DEFAULT)
        translations = self.tn_get_language(lan)

        if key not in translations:
            translations = self.tn_get_language(Settings.LANGUAGE_FALLBACK)
    ...
    def __getattr__(self, item: str) -> TranslationDict | FormatStr:
        ...
        if isinstance(value, str):
            ...
            result = TranslationDict(value)
            result._fallback = self.tn_get_language(Settings.LANGUAGE_FALLBACK)[item]
        else:
            raise UnsupportedTranslationTypeError(key=item, type=type(value), supported=[dict, str])
...
def load_translations(
    *,
    ...
    extensions: Optional[list[PrimitiveExtension]] = None,
...
t.register_translation_namespace("g", Path(__file__).parent.joinpath( "translations"))
